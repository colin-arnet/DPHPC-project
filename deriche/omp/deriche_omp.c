...
DATA_TYPE **allocarray(int n, int m)
{
    DATA_TYPE *data = (float *)malloc(n * m * sizeof(DATA_TYPE));
    DATA_TYPE **arr = (float **)malloc(n * sizeof(DATA_TYPE *));
    int i;
    for (i = 0; i < n; i++)
        arr[i] = &(data[i * m]);
...
static void __attribute__((optimize("O0"))) init_array(int w, int h, DATA_TYPE *alpha,
                                                       DATA_TYPE **imgIn,
                                                       DATA_TYPE **imgOut)
{
    int i, j;
...
// This loop produces a false result when parallelized
#pragma omp parallel for
    for (j = 0; j < _PB_H; j++)
    {
        tm1 = SCALAR_VAL(0.0);
