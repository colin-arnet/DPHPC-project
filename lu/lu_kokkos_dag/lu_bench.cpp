...
/// Benchmark for Kokkos DAG
int bench_kokkos_dag(int run_id, BenchUtil &bench, int num_runs, int matrix_size, int block_size, int num_threads)
{
    ...
    for (int i = 0; i < num_runs; i++)
    {
        ...
        randomize_matrix(A);
        bench.bench_param(run_id, "type", "Kokkos DAG");
...
int main(int argc, char **argv)
{
    ...
    for (auto config : configs)
    {
        ...
        run_id = bench_kokkos_dag(run_id, bench, num_runs, test_size, block_size, num_threads);
        printf("done\n");
    }
