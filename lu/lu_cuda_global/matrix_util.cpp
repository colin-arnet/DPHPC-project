    ...
    for (int i = 0; i < n; i++) {
        for (int k = 0; k < n; k++) {
            if (abs(result[i*n + k] - original[i*n + k]) > 0.1) {
                ...
                //std::cout << "i: " << i << " k: " << k << " NOT CORRECT: "<<  std::endl;
                std::cout <<  i << ", " << k << ", "<< abs(result[i*n + k] - original[i*n + k]) << std::endl;
                //return;
            }
            else {
                //std::cout << "i: " << i << " k: " << k << " CORRECT: "<< abs(result[i*n + k] - original[i*n + k]) << std::endl;
