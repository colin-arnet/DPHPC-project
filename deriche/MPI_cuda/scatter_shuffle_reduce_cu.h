...
struct Subarray scatter (dt **data, int h, int w) {
    ...
    if (rank == 0) {
        ...
        MPI_Recv(&(local[0][0]), send_size, MPI_FLOAT, sender, ourtag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    sub.data = local;
    sub.h = range_h;
    sub.w = w;
...
struct Subarray shuffle (struct Subarray in, int h, int w) {
    ...
    for  (int i = 0; i < size; i++) {
        ...
        MPI_Request request;
        MPI_Isend(&(in.data[0]), 1, mysubarray, i, ourtag, MPI_COMM_WORLD, &request);
        MPI_Type_free(&mysubarray);
    }
    ...
    for(int receiver = 0; receiver < size; receiver++) {
        ...
        if (receiver == rank) {
            for(int i = 0; i < range; i++) {
                for(int j = 0; j < range_w; j++) {
                    out.data[(lower + i)*range_w + j] = in.data[i*range_w + lower_w + j];
                }
            }
            continue;
        } 
        ...
        MPI_Recv(&(rec_buffer[0]), send_size, MPI_FLOAT, receiver, ourtag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        for(int i_ = 0; i_ < range; i_++) {
            for(int j_ = 0; j_ < range_w; j_++) {
                out.data[(lower + i_)*range_w + j_] = rec_buffer[i_*range_w + j_];
            }
        }
    }

    deallocarray(in.data);
    return out;
}
...
struct Subarray reduce (struct Subarray in, int h, int w) {
    ...
    if (rank != 0) {
        ...
        MPI_Send(&(in.data[0]), in.h*in.w, MPI_FLOAT, 0, ourtag, MPI_COMM_WORLD);
        deallocarray (in.data);
        ...
        for(int i_ = 0; i_ < h; i_++) {
            for(int j_ = 0; j_ < range_w; j_++) {
                out.data[i_*range_w + lower_w+j_] = in.data[i_*range_w + j_];
            }
        }
        deallocarray (in.data);
        for(int receiver = 1; receiver < size; receiver++) {
            ...
            for(int i_ = 0; i_ < h; i_++) {
                for(int j_ = 0; j_ < range; j_++) {
                    out.data[i_*range + lower+j_] = rec_buffer[i_*range + j_];
                }
            }
        }
...
dt **allocarray(int n, int m) {
    dt **data;
    cudaMalloc((void **) &data, sizeof(dt)*n*m);
    return data;
}
void deallocarray(dt** data) {
    cudaFree (data);
