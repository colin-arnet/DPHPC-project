...
static void print_array(int w, int h, ViewMatrixType imgOut)
{
  int i, j;
...
#pragma scop
  ...
  Kokkos::parallel_for(
      _PB_W, KOKKOS_LAMBDA(const int i) {
  ...
  parallel_for(
      Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {_PB_W, _PB_H}),
      KOKKOS_LAMBDA(const int i, const int j) {
        imgOut(i, j) = c1 * (y1(i, j) + y2(i, j));
      });

  Kokkos::parallel_for(
      _PB_H, KOKKOS_LAMBDA(const int j) {
