...
#include "scatter_shuffle_reduce_cu.h"
...
static
dt ** kernel_deriche(int w, int h, dt alpha, dt **imgIn, dt **imgOut) {
...
int main(int argc, char **argv) {
    ...
    dt **imgOut_host = NULL;
    dt **imgIn_host = NULL;
    dt **imgOut_device = NULL;
    dt **imgIn_device = NULL;
    ...
    if (rank == 0) {
        ...
        if (cudaMallocHost((void **) &imgIn_host, sizeof(dt)*w*h) == cudaErrorMemoryAllocation){
            cudaFreeHost(imgIn_host);
            return 1;
        }
        if (cudaMallocHost((void **) &imgOut_host, sizeof(dt)*w*h) == cudaErrorMemoryAllocation) {
            cudaFreeHost(imgIn_host);
            cudaFreeHost(imgOut_host);
            return 1;
        }
        /*
        ...
        */
        init_array(h, w, imgIn_host);
        cudaMalloc((void **) &imgIn_device, sizeof(dt)*w*h)
        cudaMalloc((void **) &imgOut_device, sizeof(dt)*w*h);

        cudaMemcpy(imgIn_device, imgIn_host, sizeof(dt)*h*w, cudaMemcpyHostToDevice);
