...
// code from here: https://newbedev.com/openmp-c-matrix-multiplication-run-slower-in-parallel
...
void mat_mult_minus_2(int n, int m, int p, const double* A, const double* B, double* result) 
{   
    double *B_trans;
    B_trans = (double*)std::malloc(sizeof(double)*m*p);
    transpose(n, p, B, B_trans);
    #pragma omp parallel
    {
        int i, j, k;
        #pragma omp for
...
void mat_mult_minus_3(int n, int m, int p, const double* A, const double* B, double* result)
{
    const int bsi = 1;
    const int bsj = 1;
    const int bsk = 1;

    for (int bj = 0; bj < m; bj += bsj) {
        for (int bi = 0; bi < n; bi += bsi) {
            for (int bk = 0; bk < p; bk += bsk) {

                for (int j = bj; j < std::min(bj+bsj, m); j++) {
                    for (int i = bi; i < std::min(bi+bsi, n); i++) {
                        for (int k = bk; k < std::min(bk+bsk, p); k++) {
                            result[i*p + k] -= A[i*m + j] * B[j*p + k];
                        }
                    }
                }
